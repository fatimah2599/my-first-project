...
print("قبل استقبال مفتاح الجلسة ")
# استقبال مفتاح الجلسة من العميل
data = client_socket.recv(1024)
encrypted_session_key = gnupg.Data(data.decode())
print("تم الاستقبال ")
# فك تشفير مفتاح الجلسة
decrypted_session_key = gpg.decrypt(encrypted_session_key, passphrase='sessionpassphrase')
print("تمالتشفير")
# إرسال موافقة على مفتاح الجلسة للعميل
client_socket.send("Session key accepted".encode())
print("تم إرسال الموافقة على مفتاح الجلسة")
# أغلق الاتصال
client_socket.close()
server_socket.close()
